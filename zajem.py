...
for kolesar in kolesarji:
    ...
    with open(ime, encoding='utf-8') as dat:
        ...
        for najdba in re.finditer(
            r'<b>Weight:</b> (?P<teza>.*?) kg .*?<b>Height:</b> (?P<visina>.*?) m<br />.*?<div class="pnt">(?P<ODR>\d*?)</div>.*?<div class="pnt">(?P<GC>\d*?)</div>.*?<div class="pnt">(?P<TT>\d*?)</div>.*?<div class="pnt">(?P<Spr>\d*?)</div>.*?<div class="pnt">(?P<Clm>\d*?)</div>',
            besedilo
        ):
            kolesar.update({"teza": najdba["teza"], "visina": najdba["visina"], "ODR": najdba["ODR"], "GC": najdba["GC"], "TT": najdba["TT"], "Spr": najdba["Spr"], "Clm": najdba["Clm"]})
